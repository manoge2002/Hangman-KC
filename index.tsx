...
const App = () => {
  ...
  return (
    <div className="min-h-screen flex flex-col items-center justify-center py-6 px-4 overflow-hidden">
      <main 
      ...
      >
        ...
        <div className="lg:col-span-3 flex flex-col items-center gap-8">
          <div className="w-full bg-slate-900/40 p-8 rounded-[3rem] border border-white/5 backdrop-blur-md flex flex-col items-center shadow-2xl">
            <HangmanDrawing wrongGuesses={wrongGuesses} />
          </div>
          <div className="w-full bg-slate-900/40 p-8 rounded-[2rem] border border-white/5 backdrop-blur-md flex flex-col items-center shadow-2xl">
            <span className="text-5xl mb-2">❤️</span>
            <span className="text-4xl font-black text-white">{Math.max(0, MAX_LIVES - wrongGuesses)} / {MAX_LIVES}</span>
          </div>
          <div 
            onClick={resetGame} 
            className="cursor-pointer text-[10px] font-bold uppercase tracking-[0.3em] text-slate-700 hover:text-slate-400 transition-colors py-2 px-4"
          >
            Spiel zurücksetzen
          </div>
        </div>
